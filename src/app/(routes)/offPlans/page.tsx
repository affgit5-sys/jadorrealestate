...
function OffPlansContent() {
  ...
  const initialDeveloperId = searchParams.get("developer_id");
  ...
  const [filters, setFilters] = useState({
    ...
    developer_id: initialDeveloperId || "any",
    bedrooms: "any",
    bathrooms: "any",
    handover_year: "any",
  });
  ...
  const fetchproperty = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const totalCount = res?.totalProjects || res?.total || 0;
  ...
  useEffect(() => {
    if (initialDeveloperId) {
      const fetchInitialDeveloperById = async () => {
        try {
          // We don't have a direct get_developer_by_id, so we search by ID if possible or just use a generic list
          // But get_developers doesn't seem to support id filter.
          // Let's check if we can find it in the search results
          const response = await api.get(`/properties/get_developers?size=100`);
          const devs = response.data?.developers || response.data || [];
          const matchedDev = devs.find((d: any) => d.id.toString() === initialDeveloperId);
          if (matchedDev) {
            setDeveloperSearch(matchedDev.name);
          }
        } catch (error) {
          console.error("Error fetching initial developer by ID:", error);
        }
      };
      fetchInitialDeveloperById();
    } else if (initialDeveloper) {
      const fetchInitialDeveloper = async () => {
        try {
          const response = await api.get(
            `/properties/get_developers?name=${initialDeveloper}`
          );
          const devs = response.data?.developers || response.data || [];
          if (devs.length > 0) {
            // Find an exact match if possible, otherwise take the first one
            const exactMatch = devs.find(
              (d: any) => d.name.toLowerCase() === initialDeveloper.toLowerCase()
            );
            handleDeveloperSelect(exactMatch || devs[0]);
          }
        } catch (error) {
          console.error("Error fetching initial developer:", error);
        }
      };
      fetchInitialDeveloper();
    }
  }, [initialDeveloper, initialDeveloperId, handleDeveloperSelect]);
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          <div className="max-w-4xl mx-auto mb-12">
            <h1 className="text-3xl md:text-4xl font-light text-gray-800 mb-6 leading-tight font-serif">
              Exclusive <span className="text-[#DC2626] font-normal">Off-Plan Projects</span> in Dubai
            </h1>
